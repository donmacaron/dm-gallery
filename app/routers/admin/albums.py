...
async def album_detail(album_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    media_items = db.query(Media).filter(Media.album_id == album_id).order_by(Media.sort_order, Media.created_at).all()
    ...
    # Latest zip job for this album
    zip_job = (
        db.query(Job)
        .filter(Job.job_type == "zip", Job.target_id == album_id)
    ...
    return templates.TemplateResponse(request, "admin/albums/edit.html", {
        ...
        "sub_albums": sub_albums, "all_albums": all_albums, "zip_job": zip_job,
    })
...
async def album_delete(album_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    album = db.query(Album).filter(Album.id == album_id).first()
    if album:
        db.query(Media).filter(Media.album_id == album_id).update({"album_id": None})
...
async def album_toggle_public(album_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    album = db.query(Album).filter(Album.id == album_id).first()
    if album:
        album.is_public = not album.is_public; db.commit()
    cls  = "badge-public" if album.is_public else "badge-private"
    text = "PUBLIC" if album.is_public else "PRIVATE"
    return HTMLResponse(
...
async def album_remove_media(album_id: int, media_id: int, request: Request, db: Session = Depends(get_db)):
    if not require_admin(request): return HTMLResponse("", status_code=401)
    media = db.query(Media).filter(Media.id == media_id, Media.album_id == album_id).first()
    if media: media.album_id = None; db.commit()
    return HTMLResponse("")
...
async def album_build_zip(album_id: int, request: Request, db: Session = Depends(get_db)):
    """Trigger background ZIP build; return HTMX polling fragment."""
    ...
    job = Job(
        id=job_id, job_type="zip", status="pending",
...
async def album_zip_status(album_id: int, request: Request, job_id: str = "", db: Session = Depends(get_db)):
    """HTMX polling fragment for ZIP build status."""
